...
class india_D90_settingsPage(Base):

    def clickAccount(self):
        """
        滑动查找Setting页面中的Account栏
    ...
    def swipeToAccountBeginning(self):
        """
        滑动至账号页面顶部
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'subList')
        pm().swipe_to_beginning_by_id(id=id)

    def swipeToAccountEnd(self):
        """
        滑动至账号页面底部
        :return:
        """
    ...
    def getAccountName(self):
        ...
        :return:
        """
        self.swipeToAccountBeginning()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountIconbg')
        iconElement = pm().find_element_by_id(id=id)
        itemInfo = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItemInfo')
        itemInfoElement = iconElement.sibling(resourceId=itemInfo)
        return itemInfoElement.get_text()

    def checkAccountBrandIsExist(self):
        """
        判断Account Brand是否存在
        :return: True元素存在，False元素不存在
        """
        self.swipeToAccountBeginning()
        element = pm().find_element_by_text(text='Brand')
        if element.exists:
            return True
        else:
            return False

    def getAccountBrand(self):
        """
        获取账号信息下Brand字段值
        :return: Brand字段值
        """
        self.swipeToAccountBeginning()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItem')
        element = pm().find_element_by_idAndText(id=id, text='Brand')
        itemInfo = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItemInfo')
        itemInfoElement = element.sibling(resourceId=itemInfo)
        return itemInfoElement.get_text()

    def checkAccountModelYearIsExist(self):
        """
        判断Account Model Year是否存在
        :return: True元素存在，False元素不存在
        """
        self.swipeToAccountBeginning()
        element = pm().find_element_by_text(text='Model Year')
        if element.exists:
            return True
        else:
            return False

    def getAccountModelYear(self):
        """
        获取账号信息下ModelYear字段值
        :return: ModelYear字段值
        """
        self.swipeToAccountBeginning()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItem')
        element = pm().find_element_by_idAndText(id=id, text='Model Year')
        itemInfo = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItemInfo')
        itemInfoElement = element.sibling(resourceId=itemInfo)
        return itemInfoElement.get_text()

    def checkAccountModelIsExist(self):
        """
        判断Account Model是否存在
        :return: True元素存在，False元素不存在
        """
        self.swipeToAccountBeginning()
        element = pm().find_element_by_text(text='Model')
        if element.exists:
            return True
        else:
            return False

    def getAccountModel(self):
        """
        获取账号信息下Model字段值
        :return: Model字段值
        """
        self.swipeToAccountBeginning()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItem')
        element = pm().find_element_by_idAndText(id=id, text='Model')
        itemInfo = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItemInfo')
        itemInfoElement = element.sibling(resourceId=itemInfo)
        return itemInfoElement.get_text()

    def checkAccountColorIsExist(self):
        """
        判断Account Color是否存在
        :return: True元素存在，False元素不存在
        """
        self.swipeToAccountEnd()
        element = pm().find_element_by_text(text='Color')
        if element.exists:
            return True
        else:
            return False

    def getAccountColor(self):
        """
        获取账号信息下Color字段值
        :return: Color字段值
        """
        self.swipeToAccountEnd()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItem')
        element = pm().find_element_by_idAndText(id=id, text='Color')
        itemInfo = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItemInfo')
        itemInfoElement = element.sibling(resourceId=itemInfo)
        return itemInfoElement.get_text()

    def checkAccountVinIsExist(self):
        """
        判断Account VIN是否存在
        :return: True元素存在，False元素不存在
        """
        self.swipeToAccountEnd()
        element = pm().find_element_by_text(text='VIN')
        if element.exists:
            return True
        else:
            return False

    def getAccountVin(self):
        """
        获取账号信息下Color字段值
        :return: Color字段值
        """
        self.swipeToAccountEnd()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItem')
        element = pm().find_element_by_idAndText(id=id, text='VIN')
        itemInfo = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItemInfo')
        itemInfoElement = element.sibling(resourceId=itemInfo)
        return itemInfoElement.get_text()

    def checkAccountRegistrationNumberIsExist(self):
        """
        判断Account Registration Number是否存在
        :return: True元素存在，False元素不存在
        """
        self.swipeToAccountEnd()
        element = pm().find_element_by_text(text='Registration Number')
        if element.exists:
            return True
        else:
            return False

    def getAccountRegistrationNumber(self):
        """
        获取账号信息下RegistrationNumber字段值
        :return: RegistrationNumber字段值
        """
        self.swipeToAccountEnd()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItem')
        element = pm().find_element_by_idAndText(id=id, text='Registration Number')
        itemInfo = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItemInfo')
        itemInfoElement = element.sibling(resourceId=itemInfo)
        return itemInfoElement.get_text()

    def checkAccountEmergencyContactPersonIsExist(self):
        """
        判断Account Emergency Contact Person是否存在
        :return: True元素存在，False元素不存在
        """
        self.swipeToAccountEnd()
        element = pm().find_element_by_text(text='Emergency Contact Person')
        if element.exists:
            return True
        else:
            return False

    def getAccountEmergencyContactPerson(self):
        """
        获取账号信息下Emergency Contact Person字段值
        :return: Emergency Contact Person字段值
        """
        self.swipeToAccountEnd()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItem')
        element = pm().find_element_by_idAndText(id=id, text='Emergency Contact Person')
        itemInfo = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItemInfo')
        itemInfoElement = element.sibling(resourceId=itemInfo)
        return itemInfoElement.get_text()

    def checkAccountEmergencyContactIsExist(self):
        """
        判断Account Emergency Contact是否存在
        :return: True元素存在，False元素不存在
        """
        self.swipeToAccountEnd()
        element = pm().find_element_by_text(text='Emergency Contact')
        if element.exists:
            return True
        else:
            return False

    def getAccountEmergencyContact(self):
        """
        获取账号信息下Emergency Contact字段值
        :return: Emergency Contact字段值
        """
        self.swipeToAccountEnd()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItem')
        element = pm().find_element_by_idAndText(id=id, text='Emergency Contact')
        itemInfo = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'accountItemInfo')
        itemInfoElement = element.sibling(resourceId=itemInfo)
        return itemInfoElement.get_text()
        ...
        for i in range(5):
            ...
            if element.exists():
                ...
                if statusText.exists():
                    ...
                    logger.info('status is not exist')
                    return None
            else:
        ...
        for i in range(5):
            ...
            if element is not None:
                ...
                if lockIcon.exists():
                    ...
                    logger.info('Lock icon is Not Exist')
                    return False
            else:
                self.swipeDownWifiConnectList()
        logger.info('WIFI Name is not Exist')
        return False
    ...
    def clickToneFieldPage(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'toneFieldPage')
        element = pm().find_element_by_id(id=id)
    ...
    def clickToneEffectPage(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'toneEffectPage')
        element = pm().find_element_by_id(id=id)
        pm().click_by_element(element=element)
    ...
    def clickVolumePage(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'volumePage')
        element = pm().find_element_by_id(id=id)
        pm().click_by_element(element=element)
    ...
    def checkCurrentPage(self, title):
        ...
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', title)
        element = pm().find_element_by_id(id=id)
        if element is not None:
            return True
        else:
    ...
    def clickClassicMode(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'classic')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickPopMode(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'pop')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickVocalMode(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'vocal')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickJazzMode(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'jazz')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickRockMode(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'rock')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickManualMode(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'manual')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickResetMode(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'reset')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def checkdToneEffectFrequency(self):
        ...
        text80 = pm().find_element_by_id(
            id=pm().readConfigByModuleAndKey('india_D90', 'settingsPage', '80Freq'))
        text500Freq = pm().find_element_by_id(id=pm().readConfigByModuleAndKey('india_D90', 'settingsPage',
                                                                               '500Freq'))
        text1kFreq = pm().find_element_by_id(
            id=pm().readConfigByModuleAndKey('india_D90', 'settingsPage',
                                             '1kFreq'))
        text5kFreq = pm().find_element_by_id(
            id=pm().readConfigByModuleAndKey('india_D90', 'settingsPage',
                                             '5kFreq'))
        text16kFreq = pm().find_element_by_id(id=pm().readConfigByModuleAndKey('india_D90', 'settingsPage',
                                                                               '16kFreq'))
    ...
    def swip80seek(self):
        ...
        :return: True滑动成功，False滑动失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', '80seek')
        element = pm().find_element_by_id(id=id)
        return pm().swipe_down_by_element(element=element)
    ...
    def swipeUpVolumePage(self):
        ...
        :return: True滑动成功 False滑动失败
        """
        xpath = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'volumeScrollViewXpath')
    ...
    def checkLoudnessStatus(self):
        ...
        :return: On 打开，Off 关闭
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'loudnessStatus')
        element = pm().find_element_by_id(id=id)
        return element.get_text()
    ...
    def openLoudnessMode(self):
        ...
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'loudnessButton')
        element = pm().find_element_by_id(id=id)  # 获取开关元素
        if element is None:  # 如果元素不存在，返回False
            return False
        else:
            statusId = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'loudnessStatus')
    ...
    def clickSystemNotificationHigh(self):
        ...
        self.swipeDownVolumePage()
        self.swipeDownVolumePage()  # 上划2次至顶部
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'notificationHigh')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickSystemNotificationLow(self):
        ...
        self.swipeDownVolumePage()
        self.swipeDownVolumePage()  # 上划2次至顶部
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'notificationLow')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickSystemNotificationOff(self):
        ...
        self.swipeDownVolumePage()
        self.swipeDownVolumePage()  # 上划2次至顶部
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'notificationOff')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickSystemBeepHigh(self):
        ...
        self.swipeDownVolumePage()
        self.swipeDownVolumePage()  # 上划2次至顶部
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'BeepsHigh')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickSystemBeepLow(self):
        ...
        self.swipeDownVolumePage()
        self.swipeDownVolumePage()  # 上划2次至顶部
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'BeepsLow')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def findSVCGroup(self):
        ...
        self.swipeDownVolumePage()
        self.swipeDownVolumePage()  # 上划2次至顶部
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'svcId')
        element = pm().find_element_by_id(id=id)
        if element is None:
    ...
    def clickSVCHigh(self):
        ...
        """
        self.findSVCGroup()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'svcHigh')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickSVCNormal(self):
        ...
        """
        self.findSVCGroup()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'svcNormal')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickSVCLow(self):
        ...
        """
        self.findSVCGroup()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'svcLow')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickSVCOff(self):
        ...
        """
        self.findSVCGroup()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'svcOff')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def naviVolumeMax(self):
        ...
        self.swipeUpVolumePage()
        self.swipeUpVolumePage()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'naviBar')
        element = pm().find_element_by_id(id=id)
        pm().swipe_right_by_element(element=element)
        volumeId = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'navVolume')
    ...
    def mediaVolumeMax(self):
        ...
        self.swipeUpVolumePage()
        self.swipeUpVolumePage()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'mediaBar')
        element = pm().find_element_by_id(id=id)
        pm().swipe_right_by_element(element=element)
    ...
    def phoneVolumeMax(self):
        ...
        self.swipeUpVolumePage()
        self.swipeUpVolumePage()
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'phoneBar')
        element = pm().find_element_by_id(id=id)
        pm().swipe_right_by_element(element=element)
        volumeId = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'phoneVolume')
        elementVolume = pm().find_element_by_id(id=volumeId)
        if elementVolume.get_text() == '32':
            return True
        else:
    ...
    def mediaStepIncrease(self):
        ...
        if mediaVolumeBeforeChange == 32:  # 初始值最大的情况下最值判断
            if mediaVolumeAfterChange == mediaVolumeBeforeChange:
                ...
                logger.info(mediaVolumeBeforeChange)
                logger.info(mediaVolumeAfterChange)
                return False
    ...
    def clickFactoryResetButton(self):
        """
        点击FactoryReset开关
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'factoryResetButton')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickSystemResetButton(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'systemResetButton')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)
    ...
    def clickVehicleResetButton(self):
        ...
        :return: True点击成功，False点击失败
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'vehicleResetButton')
        element = pm().find_element_by_id(id=id)
        return pm().click_by_element(element=element)

    def checkFactoryResetText(self):
        """
        检查；Factory Reset话术
        :return: True话术正确，False话术错误
        """
        id = pm().readConfigByModuleAndKey('india_D90', 'settingsPage', 'resetDialog')
        element = pm().find_element_by_id(id=id)
        if element.get_text() == 'Do you want to do factory reset?':
            return True
        else:
            return False
    ...
    def changeToSettingBTPageFromMorePage50times(self):
        ...
        for i in range(50):
            ...
            if settingPageActivity == 'com.saicmotor.settings.ui.MainActivity':
                ...
                logger.info(str(i) + ' Fail ' + settingPageActivity)
                k = k + 1
...
settingsPage = india_D90_settingsPage()  # 单例
