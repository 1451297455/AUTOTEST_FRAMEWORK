...
class PCANBasic:
    ...
    def __init__(self):
        # Load the PcanBasic依赖库
        #
        if platform.system() == 'Windows':
            self.__m_dllBasic = windll.LoadLibrary("Pconfig/PCANBasic32")
        elif platform.system() == 'Darwin':
            self.__m_dllBasic = cdll.LoadLibrary("Pconfig/libPCBUSB.dylib")
        else:
            self.__m_dllBasic = cdll.LoadLibrary("Pconfig/libpcanbasic.so")
        if self.__m_dllBasic == None:
            print("Exception: The PCAN-Basic lib couldn't be loaded!")
